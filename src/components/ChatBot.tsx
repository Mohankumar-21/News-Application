...
const ChatBot: React.FC<ChatBotProps> = ({ closeChat }) => {
  ...
  const fetchBotResponse = async (query: string) => {
    ...
    try {
      ...
      if (data.articles && data.articles.length > 0) {
        ...
        setMessages((prevMessages) => [...prevMessages, { type: 'bot', text: 'Sorry, I couldn’t find any news articles related to that.' }]);
      }
    } catch (error) {
      console.error("Error fetching news data:", error);
      setMessages((prevMessages) => [...prevMessages, { type: 'bot', text: 'There was an error retrieving the news.' }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed bottom-10 right-10 w-80 h-96 bg-gray-800 text-white shadow-lg rounded-lg p-4">
      <button onClick={closeChat} className="absolute top-2 right-2 bg-red-600 rounded-full h-8 w-8 p-1 text-white">X</button>
      <div className="overflow-y-auto h-72 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
        {messages.map((msg, index) => (
          <div key={index} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'} mb-2`}>
            <div className={`max-w-[75%] ${msg.type === 'user' ? 'text-right' : 'text-left'} px-2 py-1 overflow-hidden`}>
              <p className={msg.type === 'user' ? 'text-blue-400 text-xs' : 'text-gray-200 text-xs text-pretty'}>{msg.text}</p>
            </div>
          </div>
        ))}
      ...
      <div className="flex mt-2">
        ...
        />
        <button onClick={handleSend} className="bg-blue-600 text-white p-2 rounded-r text-sm">Send</button>
      </div>
    </div>
  );
};
